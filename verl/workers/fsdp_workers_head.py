...
def apply_head_only(model, cast_head_to_fp32: bool = False):
    print("Enabling head-only training")

    output_head = model.get_output_embeddings()
    if output_head is None:
        raise ValueError("Model has no output embeddings; cannot do head-only training.")

    # 1) 如果 lm_head 和 input embedding 绑定在一起，先 untie
    if getattr(model.config, "tie_word_embeddings", False):
        print("Untying output head from input embeddings")

        head_weight = output_head.weight
        head_bias = getattr(output_head, "bias", None)
        has_bias = head_bias is not None

        # 直接按当前 head 的 device / dtype 新建，避免依赖 next(model.parameters()).device
        ...
        # 只有在非 meta tensor 时才真实拷贝
        if not head_weight.is_meta:
            with torch.no_grad():
                new_head.weight.copy_(head_weight.detach())
                if has_bias:
                    new_head.bias.copy_(head_bias.detach())
    ...
    # 3) 只解冻输出头
    output_head.requires_grad_(True)

    # 4) 显式确保输入 embedding 仍冻结
    input_emb = model.get_input_embeddings()
    if input_emb is not None:
        input_emb.requires_grad_(False)

    # 5) 可选：将输出头转为 fp32
    # 仅在非 meta tensor 时做
    if (
        cast_head_to_fp32
        and hasattr(output_head, "weight")
        and output_head.weight is not None
        and not output_head.weight.is_meta
        and output_head.weight.dtype != torch.float32
    ):
        print(f"Converting output head from {output_head.weight.dtype} to float32")
        output_head.to(dtype=torch.float32)
    else:
        if hasattr(output_head, "weight") and output_head.weight is not None:
            print(f"Output head dtype kept as {output_head.weight.dtype}")
    ...
    if input_emb is not None:
        input_trainable = any(p.requires_grad for p in input_emb.parameters())
        if input_trainable:
            print("WARNING: Input embeddings are still trainable!")
        else:
            print("✓ Input embeddings are properly frozen")

    output_trainable = any(p.requires_grad for p in output_head.parameters())
    if not output_trainable:
        raise RuntimeError("Output head is not trainable - head-only training failed!")
    else:
        print("✓ Output head is properly trainable")
...
class FSDPWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        if self._is_actor and os.getenv("VERL_HEAD_ONLY_TRAINING") == "1":
            self.print_rank0("Applying head-only transformation to ACTOR only")
            model = apply_head_only(model, cast_head_to_fp32=False)
        ...
        if self._is_actor or self._is_critic:
            trainable_params = [p for p in self.fsdp_module.parameters() if p.requires_grad]
